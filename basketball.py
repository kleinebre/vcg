...
yourshot:
	...
	IF RND(1)<.5 THEN GOSUB 1000:GOTO center_jump
	IF T<100 THEN GOSUB 1000: GOTO center_jump
	PRINT
...
1000 IF Z<>1 AND Z<>2 THEN GOSUB 1300: RETURN
	T=T+1
	IF T=50 THEN GOSUB end_of_first_half: RETURN
	IF T=92 THEN GOSUB two_minutes_left
	PRINT "JUMP SHOT"
	IF RND(1)<=.341*D/8 THEN PRINT "SHOT IS GOOD.":GOSUB 7000:GOSUB 3000: RETURN
	IF RND(1)>.682*D/8 THEN GOSUB 1200: RETURN
	PRINT "SHOT IS OFF TARGET."
	IF D/6*RND(1)>.45 THEN PRINT "REBOUND TO ";O$:GOSUB 3000: RETURN
	PRINT "DARTMOUTH CONTROLS THE REBOUND."
	IF RND(1)<=.4 THEN GOSUB 1300: RETURN
	IF D<>6 OR RND(1) <= .6 THEN PRINT "BALL PASSED BACK TO YOU. "; :YS=1: RETURN
	PRINT "PASS STOLEN BY ";O$;" EASY LAYUP.":GOSUB 6000:PRINT: YS=1: RETURN

1200 IF RND(1)>.782*D/8 THEN
        IF RND(1)>.843*D/8 THEN PRINT "CHARGING FOUL.  DARTMOUTH LOSES BALL.":GOSUB 3000: RETURN
	    PRINT "SHOOTER IS FOULED.  TWO SHOTS.":GOSUB 4000:GOSUB 3000: RETURN
    END IF
    PRINT "SHOT IS BLOCKED.  BALL CONTROLLED BY ";
    IF RND(1)<=.5 THEN PRINT O$;".":GOSUB 3000: RETURN
    PRINT "DARTMOUTH."
    YS=1:
	RETURN
...
1300 T=T+1
	IF T=50 THEN GOSUB end_of_first_half: RETURN
	IF T=92 THEN GOSUB two_minutes_left
	IF Z=0 THEN GOSUB 2010:PRINT: YS=1: RETURN
	IF Z>3 THEN PRINT "SET SHOT." ELSE PRINT "LAY UP."
	IF 7/D*RND(1)<=.4 THEN PRINT "SHOT IS GOOD.  TWO POINTS.":GOSUB 7000:GOSUB 3000: RETURN
	IF 7/D*RND(1)>.7 THEN GOSUB 1500: RETURN
	PRINT "SHOT IS OFF THE RIM."
	IF RND(1)<=2/3 THEN PRINT O$;" CONTROLS THE REBOUND.":GOSUB 3000: RETURN
	PRINT "DARTMOUTH CONTROLS THE REBOUND."
	IF RND(1)<=.4 THEN GOTO 1300
...
7500 PRINT "SCORE: ";S(1);"TO";S(0)
	RETURN
end_of_first_half:
    PRINT:PRINT "   ***** END OF FIRST HALF *****":PRINT
